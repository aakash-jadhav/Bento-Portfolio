      ...
      <aside className="flex shrink-0 flex-col gap-2 overflow-visible rounded-xl bg-white p-4 shadow-sm shadow-slate-900/5 lg:p-4">
        ...
        <nav className="flex flex-col items-stretch gap-1.5 overflow-visible pr-0.5">
          {titleList.map((title, idx) => {
            ...
            const previewMatchBg = getProjectSidebarActiveBgClass(idx)
            return (
              <button
                key={`${title}-${idx}`}
                type="button"
                onClick={() => setSelectedIndex(idx)}
                className={`flex w-full cursor-pointer items-center py-2 pl-3 pr-4 text-left text-sm font-medium transition ${
                  active
          ...
          <div className="mt-6 space-y-4">
            ...
            <Field label="Tone">
              <select
                ...
                value={selected.tone}
                onChange={(e) =>
                  updateSelected({ tone: e.target.value as TagTone })
                }
              >
                {TONE_OPTIONS.map((t) => (
